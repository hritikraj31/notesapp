...
function execCommandWithArg(command, arg) {
	if (arg) texteditor.document.execCommand(command, false, arg);
}

function toggleSource() {
	if (showingSourceCode) {
		texteditor.document.getElementsByTagName(
			'body'
		)[0].innerHTML = texteditor.document.getElementsByTagName(
			'body'
		)[0].textContent;
		...
		texteditor.document.getElementsByTagName(
			'body'
		)[0].textContent = texteditor.document.getElementsByTagName(
			'body'
		)[0].innerHTML;
...
function submitNote(idname) {
	if (
		texteditor.document.getElementsByTagName('body')[0].innerHTML.length <= 0
	) {
	...
	if (description.value.length <= 0) {
		description.value = texteditor.document
			.getElementsByTagName('body')[0]
			.textContent.substring(0, 75);
	}
	document.getElementById(
		idname
	).value = texteditor.document.getElementsByTagName('body')[0].innerHTML;
}
