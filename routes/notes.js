...
router.get('/', middleware.isLoggedIn, function (req, res) {
	noteslists.findOne({ author: req.user._id }, function (err, noteslist) {
		if (err) {
			...
			if (noteslist) res.render('notes/index', { notelists: noteslist.list });
			else res.render('notes/index', { notelists: noteslist });
		}
	});
});
...
//new post route
router.post('/', middleware.isLoggedIn, middleware.isEmptyNote, function (
	req,
	res
) {
	var newNote = {
		...
		if (err) {
			...
			noteslists.find({ author: req.user._id }, function (err, notelist) {
				if (err) {
					...
					noteslists.create({ author: req.user._id, list: list }, function (
						err,
						createNotelist
					) {
						if (err) {
...
//Edit Put Route
router.put('/:id', middleware.isLoggedIn, middleware.isEmptyNote, function (
	req,
	res
) {
		...
		if (err) {
			...
			noteslists.findOne({ author: req.user._id }, function (err, notelist) {
				if (err) {
					...
					notelist.list = notelist.list.filter(function (a) {
						return !a.id.equals(req.params.id);
					});
					notelist.save(function (err) {
						if (err) {
							console.log(err);
